...
<div class="absolute md:fixed w-full flex items-center h-14 text-white z-50">
    <div class="flex items-center justify-around pl-2 md:px-3 w-full h-14 bg-blue-700 dark:bg-gray-600 border-none">
      <div class="flex">
        <!-- <img class="w-7 h-7 mr-2 rounded-md overflow-hidden hidden processing" src={dummyAvatar} alt="profile" /> -->
        ...
        <a href="/" on:click={() => itemHeaderselected(ALL)}>
          <img class="w-24 h-24 bg-transparent hidden md:block" src={logoWhite2} alt="logo">
        </a>
      </div>
      <div class="flex items-center">
        <a href="/" on:click={() => itemHeaderselected(ALL)}>
          <img class="w-24 h-24 bg-transparent md:hidden" src={logoWhite2} alt="logo">
        </a>
      ...
      <div class="flex items-center">
          ...
          >
          {#if isDark}
            <svg class="w-6 h-6 fill-current text-gray-700 group-hover:text-gray-500 group-focus:text-gray-700 dark:text-gray-700 dark:group-hover:text-gray-500 dark:group-focus:text-gray-700" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="" >
